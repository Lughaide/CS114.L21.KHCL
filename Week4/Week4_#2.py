# Source: https://codereview.stackexchange.com/questions/178772/codefights-pipes-game
...
def follow_flow(flow, current_path, state, end, final_path):
    ...
    if in_range(x, y, state) and state[x][y] == end:
        final_path.append(current_path + [''])
    elif in_range(x, y, state) and state[x][y] in FLOW[flow]:
...
def sum_water(total_path):
    ...
    final_cells = set()
    first_leak = list(map(len, filter(lambda x: 'leak' in x, total_path)))
...
def pipesGame(state):
    ...
    for pipe in pipes:
        ...
        for di in DIRECTION:
            ...
            if in_range(x, y, state) and state[x][y] in FLOW[di]:
...
game_state = [stdin.readline().strip('\n') for i in range(n)]
stdout.write(str(pipesGame(game_state)))
